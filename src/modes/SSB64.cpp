...
SSB64::SSB64(socd::SocdType socd_type, bool enable_cbutton_aerials) {
    socd_type = socd::SOCD_NEUTRAL;
    _socd_pair_count = 4;
    ...
    _cbutton_aerials = enable_cbutton_aerials;
    _horizontal_socd = false;
}

void SSB64::HandleSocd(InputState &inputs) {
    _horizontal_socd = inputs.left && inputs.right;
    InputMode::HandleSocd(inputs);
}

void SSB64::UpdateDigitalOutputs(InputState &inputs, OutputState &outputs) {
    outputs.a = inputs.a;
    outputs.b = inputs.b;

    // SSB64: Remap grab, taunt and shield to the right buttons
    outputs.buttonR = inputs.l || inputs.r;
    outputs.triggerLDigital = inputs.dpad_up;
...
void SSB64::UpdateAnalogOutputs(InputState &inputs, OutputState &outputs) {
    ...
    */
    ...
    // SSB64: Handle jump buttons
    outputs.rightStickX = inputs.x ? ANALOG_STICK_NEUTRAL + 112 : outputs.rightStickX;
    outputs.rightStickY = inputs.y ? ANALOG_STICK_NEUTRAL + 112 : outputs.rightStickY;    

    // SSB64: c-button aerials (NOT TOURNAMENT LEGAL)
    if (_cbutton_aerials)
    {
        outputs.leftStickX = 128 + (directions.cx * 53);
        outputs.leftStickY = 128 + (directions.cy * 53);
        if (outputs.leftStickX != 0 || outputs.leftStickY != 0)
        {
            outputs.a = true;
        } 
    }
